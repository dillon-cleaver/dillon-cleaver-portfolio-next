describe('Accessibility and SEO', () => {
  ...
  it('should have no console errors', () => {
    ...
    // Filter out React/Next.js framework-level hydration warnings — these are
    // not application errors and are already suppressed for uncaught exceptions.
    cy.get('@consoleError').then((spy) => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const calls = (spy as any).args as unknown[][];
      const appErrors = calls.filter(
        (args) =>
          !String(args[0] ?? '').includes('hydrated but some attributes') &&
          !String(args[0] ?? '').includes('Hydration failed') &&
          !String(args[0] ?? '').includes('hydration mismatch')
      );
      expect(
        appErrors,
        `unexpected console.error: ${JSON.stringify(appErrors)}`
      ).to.have.length(0);
    });
  });
});
